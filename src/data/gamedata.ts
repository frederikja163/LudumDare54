...
export class GameData {
    ...
    public readonly workersTotal = value(6);
    ...
    public readonly tileCapacity = equation([this.workersTotal], n => (4 * n[0]));
    public readonly workersActive = equation([this.totalTiles, this.workersTotal], n => Math.max(Math.round(n[0] / 4), n[1]));
    ...
    public readonly foodProduction = equation([this.farmerProduction, this.farmersTotal, this.farmProduction, this.farmTiles, this.queensTotal], n => Math.min(n[0] * n[1], n[2] * n[3]) + 5 * n[4]);

    // ants/min
    public readonly antProduction = equation([this.queenTiles], n => n[0] < 6 ? 1 : Math.log(n[0] - 5) / Math.log(2));
